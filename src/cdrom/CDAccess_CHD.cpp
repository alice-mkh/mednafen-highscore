...
bool CDAccess_CHD::Load(VirtualFS* vfs, const std::string &path, bool image_memcache)
{
  ...
  while (1)
  {
    ...
    if (strcmp(type, "MODE1") && strcmp(type, "MODE1_RAW") && strcmp(type, "MODE2_RAW") &&
        strcmp(type, "AUDIO"))
    {
      throw MDFN_Error(0, _("chd_parse track type %s unsupported"), type);
    }

    if (strcmp(subtype, "NONE"))
    {
      throw MDFN_Error(0, _("chd_parse track subtype %s unsupported"), subtype);
    }
...
bool CDAccess_CHD::Fast_Read_Raw_PW_TSRE(uint8_t *pwbuf, int32_t lba) const noexcept
{
  int32_t track;
